...
def sample_to_prompt_completion(
        sample: dict,
        attribute: str='sentiment',
    ) -> dict:
    """
    Extract the prompt and completion for the given sample and attribute.

    Args:
        sample (dict): The sample to extract the prompt and completion from.
        attribute (str): The attribute to extract the prompt and completion for.

    Returns:
        dict: The prompt and completion for the given sample and attribute.
    """
...
if __name__ == "__main__":
    ...
    train_dataset = train_dataset.map(sample_to_prompt_completion, fn_kwargs={
        'attribute': config['attributes'][0],  # for now we only support one control attribute
    })
    eval_dataset = eval_dataset.map(sample_to_prompt_completion, fn_kwargs={
        'attribute': config['attributes'][0],  # for now we only support one control attribute
    })
    ...
    if config['lora'].get('module_path', None):
        model = PeftModel.from_pretrained(model, config['lora']['module_path'])
    else:
